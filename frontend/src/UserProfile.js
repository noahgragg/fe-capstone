...
export const UserProfile = ({currentUserId, keys}) => {
  const [currentUserInfo, setCurrentUserInfo] = useState([{}]);
  useEffect(()=>{
    fetch(`${keys.dataURL}/api/data/${currentUserId}`)
    .then(res => res.json())
    .then(data => {
  ...
  useEffect(()=>{
    fetch(`${keys.dataURL}/api/data/${currentUserId}/project`)
    .then(res => res.json())
    .then(projects => {
      ...
      <div className='user-profile-body'>
        <div clasName='user-profile-info'>
          ...
          <div className='user-profile-photo'>
            <img src={currentUserInfo.profile_image == 'No Image' ? profile_img : currentUserInfo.profile_image} width='200px'/>
          </div>
          ...
          <a className='linked-link' href={currentUserInfo.linked_link}>LinkedIn Profile</a>
          </div>
          <div className='user-profile-resume'>
            <a className='resume-link' href={currentUserInfo.github_link}>Github Profile</a>
          </div>
        </div>
